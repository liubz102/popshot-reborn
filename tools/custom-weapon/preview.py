...
⚠ **一列 = 把那一个变体整体施加到所有素材上**，而实装是**分桶**的：`spec.Batch` 的
  `variant`（本体：手持件 / HUD 图标 / 商店图标）和 `fx_variant`（特效：弹体 / 碎片 / 拖尾 /
  特效贴图 / efx 颜色）可以不同 —— C 批现在就是**枪身 `G4` + 特效 `C`**（D44）。
  ⇒ 想看实装后的样子：**枪身看 `G4` 那列、特效看 `C` 那列**，别把某一列整列当成成品。
  `preview_sprites_*` 那张混了两桶（图标是本体、弹体是特效），尤其别整列读。
