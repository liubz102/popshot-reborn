...
5. **不启动 `NMService.exe`；`BsPatcherChn.exe` 已不是 NGM（2026-08-22 起）。**
   ...
   本体不需要，继续别碰。`game_patched\BsPatcherChn.exe` 是**自研更新器**
   （`updater\src\` 独立 C 工程的编译产物，会话 47 起**全逻辑在此 exe 内**：
   原版风格界面 + 探针/下载/应用，python 完全退出更新链）：客户端升级分支
   拉起它、触发自动更新是预期行为，不再属于禁用项。原版 NGM 链
   （BsPatcherChn→NGMDll→platform.tiancity.com）整条废弃；`NGMResource.dll`
   的界面素材已提取进 `updater\ui\`（工程资源），DLL 本体留在包里没删。
