...
## 230. ★★★★ 地上捡到的金币，客户端**一分钱都不加** —— 面额和记账全是服务端的活
...
§230 查明「面额是服务端的活」之后，剩下的问题是「原版一局掉几枚」。
...
- **面额可以按枚数反推**：通关人均 20 枚金币×1，所以
  「想让通关金币雨值 N 金币」⇒ 金币×1 的面额取 `N / 20`。
- ⚠ **实际到手会比理论少**：撒币要 `(人数+5)` 秒，而客户端在通关
  **6 秒后**就发 `0x040f` 弹结算（§167）。两人以上时尾巴那段来不及捡：
  能捡的大约是 `20×n × 6/(n+5)` 枚（n=1 全部 20、n=3 约 45/60、n=6 约 65/120），
  而且还得真的走过去踩到。**最终要拿实机日志里 `捡到 N` 那一格标定。**
