...
## 241. ★★★★★ 换图**不会**重置角色的死亡次数 —— `0x47900a` 把六个座位的角色原样挂回去（bug调查/12，V0.2 会话 51）

用户实机报（熔岩巨龙 `Quest02`，日志在 `bug调查/12/logs`，2026-08-23 23:52 那一局）：

1. 第一张图死一次，右上角剩 2 颗心 —— 对。
2. **换图进 boss 房间再死一次，心形还是 2 颗**（应该 1 颗）。
3. **在 boss 房间第三次被打死，人躺在地上再也起不来。**

### 日志证据（`bug调查/12/logs/server.out`）

```text
23:54:15.104  HP 归零上报: 玩家座位 0 句柄=0x000186a1 死亡次数=0  -> 回 0x0406 死亡次数 -> 1
23:55:04.312  换图请求: 下一张地图 = 'Quest02_2'
23:57:51.649  HP 归零上报: 玩家座位 0 句柄=0x000186a1 死亡次数=1  -> 回 0x0406 死亡次数 -> 1   ★ 该是 2
23:58:48.385  HP 归零上报: 玩家座位 0 句柄=0x000186a1 死亡次数=1  -> ★★ 一个包都没回
23:59:44.355  对端关闭（人卡了 56 秒之后自己退了）
```

★ **23:57:51 那一发上报的死亡次数是 1，不是 0** —— 这就是硬证据：
换过一张图之后，客户端角色的 `[char+0x600]`（= HUD 心形的数据源，§109）
**还是 1**，根本没有归零。

### 静态复核 `0x47900a`（`gspRepChangeToNextMap` 的处理器往下）

反汇编（`re/BigShot_22524.img`）逐段：

```asm
00479039  loop seat=0..5:  call 0x404ff6            ; 座位 -> 角色对象
          mov [ebp+esi*4-0x7c], eax                ; ★ 把指针存进栈上数组
          push [eax+0xd0] / push [0x72e2d4] / call 0x474029
          push 0 / call 0x477688                   ; 从世界里摘下来
0047906F  push 0x78 / push [0x72e2d4]+0x3c / lea eax,[ebp-0x110] / call 0x5f55f0
                                                   ; ★ 备份 0x78 字节 = 30 个 dword
0047908B  loop seat=0..5:  call 0x404ff6 -> vf[+0x14c] / vf[+0x150]   ; 卸场景
004790C3  loop seat=0..5:  edi = [ebp+esi*4-0x7c]  ; ★★ 同一批指针
          push edi / call 0x473e7c                 ; 原样挂回世界
          push edi / call 0x477688
004790FC  loop 30 次:  ecx = 备份[i]
          cmp [eax], ecx / jge skip / mov [eax], ecx  ; ★ 按 max 合并回去
```

**角色对象不是重建的，是「摘下来 → 卸场景 → 原样挂回去」**，指针一个都没变。
`[0x72e2d4]+0x3c` 起的 30 个 dword 还被显式地按 `max` 合并回去 ——
换图**根本不是**一次干净的重置，客户端是**刻意**让每座位的账目跨图累计的
（本来就该如此：闯关的三颗心是一整轮的，不是一张图的）。

### 服务端错在哪

`RoomQuest.begin_map_change` 把 `death_counts` / `dead_events` /
`last_death_broadcast_at` **一律清空**，依据是一句想当然的注释
「新角色的 `[char+0x600]` 从 0 起」。清掉之后一发引出两个症状：

| 第几次死 | 客户端报 | 服务端权威计数 | 下发 | 后果 |
|---|---|---|---|---|
| 图 2 第 1 次（全局第 2 次）| 1 | `0 + 1 = 1` | **1** | `[char+0x600]` 被写回 1，心形纹丝不动 |
| 图 2 第 2 次（全局第 3 次）| **还是 1** | —— | **不发** | `(句柄, 1)` 撞上上一发的老键 -> 判成重复上报 -> `Character::Die()` 永远不被调用 -> **人躺着起不来** |

第二格的「客户端还是报 1」是第一格的直接后果：`[char+0x600]` 只由我们的
`0x0406` 写（§109），我们回了 1，它就还是 1。**一处错，错两次。**

### 修法与边界

`begin_map_change` 改成**只清怪 / 场景物件那一份**，玩家句柄的三份计数留下
（靠 `RoomQuest.player_handles`：句柄 -> 座位，在 `record_death` 里按
`0x0408` 报的座位号登记，不去算 `座位×100000+100001` 那条公式）。

- 怪 / 物件照旧全清 —— 它们是真的重建，旧句柄真的作废。
- 玩家的**过期重复上报**照旧被吃掉：`dead_events` 里玩家那几把键也留着。
- `respawn_due` / `respawn_hints` 照旧全清 —— 换了图，旧坐标就是别的地方了。

回归：`server/test_battle.py` 三条新用例（换图后心形要减、换图后第三次死亡
仍要广播且剩余生命归零、换图后的过期重发仍要被吃掉），旧行为下三条全红。

### 顺带查明：闯关的「最大生命」不是写死的 3

`GameContext::vf_a0/vf_a4` 都转发给 `[ctx+0x384]` 的胜负条件对象
（`GameContextWaitingRoom` 那种没有胜负条件的返回 `0x7fffffff` = 无限命）：

| 胜负条件类 | vft | 最大生命 `vf+0x0c` | 剩余生命 `vf+0x10` |
|---|---|---|---|
| `QuestVictoryCondition` | `0x6935dc` | `0x55e095` = **`[vc + 座位*4 + 0x198]`（逐座位，来自关卡脚本）** | `0x55e0a3` |
| `SurvivalVictoryCondition` | `0x693624` | `0x55db69` = **写死 3** | `0x55e0a3` |
| `DeathMatch` / `NewDeathmatch` / `TimeAttack` / `Debug` | —— | `0x55e0d1` = `0x7fffffff` | 同左（**无限命**）|

`0x55e0a3`（剩余生命）= `最大生命(座位) - [vc + 座位*0x2c + 0x60]`，负数截 0。
`[vc + 座位*0x2c + 0x60]` 由 `0x48c942` 在**每收到一发玩家的 `0x0406`** 时 `+= 1`
—— 和 `[char+0x600]` 是**两个不同的计数器**：前者管「还能不能重生」
（`Die()` 在 `0x501976` 读它，为 0 就把 `[char+0x2d8]` 写成 -1，永不重生），
后者管「HUD 画几颗心」。两者都得靠我们的广播喂，**漏发一发两个都会歪**。

⇒ **服务端不知道闯关这一关给几条命**（在关卡脚本里）。所以重生看门狗那道
「还有命才补」的闸只对生存 / 夺分模式生效（那里 3 是客户端写死的），
闯关不加 —— 加了就是拿 3 去猜一个我们并不知道的数（D162）。
