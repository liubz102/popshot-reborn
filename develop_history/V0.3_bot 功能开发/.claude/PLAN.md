...
- ★★ **身体朝向必须改成「面朝准星」**（用户 2026-08-26 提醒）。
  这个游戏的朝向跟的是**鼠标准星**、不是移动方向（§37），所以
  「一边往后退一边朝身后开枪」是**合法且常见**的姿势 —— 收方那边就是
  `Run-B%02d` 那条分支（§39）。现在 `botsync.aim_point()` 把准星摆在 bot
  **正前方**，所以它永远是朝前走；M5 有了目标之后，那个函数**换成敌人
  （或瞄准点）的世界坐标**即可，朝向位 / 角度 / `Run-F` vs `Run-B`
  会由 `aim_state()` 自动跟着变，不用再动别的地方。
