...
最后更新：2026-08-26（会话 11）

---

## 现在的位置

**★ 走路动画已实机通过（§39）。之后用户按「移动能力」逐项点名，补了两档：**

| 用户问的 | 结论 | 状态 |
|---|---|---|
| 右键**冲刺**时 bot 没有扬尘 | 扬尘**原版就只有自己看得见**（§40）；但**冲刺位 bit3 确实漏发了** —— 它管位移速度和腿的动画速率（`dt × FastRunRate` = 1.5） | 已补（会话 10）⏳ 等复验 |
| 下键**蹲下** + 体力恢复加快，实装了吗 | **没实装**。蹲**不在心跳里**，是单独的事件包 `rpCrouch`(0x000b)（§41）；那一位在收方管姿势 + **速度 × 1/3** + **体力恢复 × 2** | 已补（会话 11）⏳ 等复验 |

两档都是同一个坑的两个方向：**收方是照着这些状态位自己替角色算位移的**，
状态位和坐标的实际步长对不上就会「推一下、拽一下」。

M4 已完成并通过实机核对。M3b（开火）仍未开始。
...
| M3a | bot **会动会跳**（`botsync.py` 合成器 + 帧驱动） | 🟢 **会动会跳、跳跃流畅、身体朝向、走路动画全部实机通过**；冲刺（§40）和蹲下（§41）刚补，⏳ **等复验** |
...
## ⏳ 待用户实机复验（会话 10 + 11：冲刺 / 蹲下）

★ **先重启服务端**（铁律 7）。房主是你，`/bot` 加 1 个 bot，`/ready`，按开始。

| # | 步骤 | 期望看到 |
|---|---|---|
| 1 | ★★★ **按住右键加速跑一段长的**（能量够的话跑久一点） | bot **跟得上**，腿的动画也跟着变快；不出现「越落越远、然后一顿一顿被拽上来」 |
| 2 | ★ 加速跑和普通走**交替** | 切换那一下不抽、不打滑 |
| 3 | ★★★ **按下键蹲下**，停一会儿再站起来 | bot 走到你身后也**蹲下**（蹲姿），你起立它也起立 |
| 4 | ★ **蹲着走一段**（蹲下 + 左右键） | bot 跟着蹲走，速度慢下来但**不打滑、不被拽**（蹲走是 1/3 速度）|
| 5 | ★ 蹲着的时候**换图 / 打死它 / 打完一局再开一局** | 新一局 bot 站姿正常，不会一直保持蹲着 |
| 6 | 扬尘 | ★ **仍然不会有**（原版行为，§40）—— 不是 bug，不用再看 |
| 7 | 普通走 / 站住 / 跳 / 朝向 / 进度条 | 照旧正常（回归看一眼）|

★ **bot 还是不会开枪**（M3b，D18），只跑不打是预期。

**怎么告诉我结果**：第 1 / 3 / 4 条各回一句就够。不行的话请贴
...
| 位域 `bit3`（`[char+0x4bc]`）| ✅ **冲刺**（§40：`dt × FastRunRate`，实测 1.5 倍）。bot 已跟着真人报，⏳ 实机复验中 |
| 位域 `bit4`（`[char+0x2dc]`）/ `bit5`（`[char+0x59c]`）| ❓ bit5 几乎不出现、bit4 一半一半看不出规律。bot 全填 0，实机没见到问题 |
| 蹲下 `[char+0x2b5]` | ✅ **查明并实装**（§41：事件包 `rpCrouch`(0x000b)，收方姿势 + 速度 ×1/3 + 体力恢复 ×2）。⏳ 实机复验中 |
