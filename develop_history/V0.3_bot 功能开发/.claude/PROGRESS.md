...
## 下一步该做什么（M3 · bot 会动会开枪）

★ 不依赖上面那轮验证，可以现在就开工（逆向是离线的）。

1. 逆 `0x4001` 心跳 body 剩下的字段（§3 已有大半，要跟反汇编核对）。
2. 逆 `rpFire`(0x0002, 26B) / `rpExplode`(0x0003, 28B) / `rpJump`(0x0006, 2B) /
   `rpChangeWeapon`(0x0001, 5B)。语料 `bug调查/server_logs/game_003_27799.dec.bin`（§2）。
3. 新文件 `server/botsync.py` 合成 `UdpPacket`，**三条不变式写成断言**（D5）。
   校验和直接抄 `tools/fakeclient.py` 的 `udp_checksum()`（§4，25091/25091 实测全中）。
4. 投递走现有 `relayserver.deliver()`，**不新增第二条路**。

**开工前先读**：`FINDINGS.md` §2 / §3 / §4、`DECISIONS.md` D5、
`re/packet_api.md` §5。
