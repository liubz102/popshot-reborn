...
最后更新：2026-08-27（会话 21）

---

## 现在的位置

### ⏳ **等用户实机验收**（会话 19 + 20 + 21 攒在一起）
...
- ⬜ **地面燃烧（火墙）**：`rpSetOnFire`（`0x0005`）的布局已经逆完
  （§69 / packet_api §5.4d）。★ `FlamingBottle` 会话 21 已经回到 `usable`
  （§72），但它现在**只是一颗普通手雷** —— 火墙要靠射手自己发
  `rpSetOnFire`（那一段在收方是 `IsMine` 门里的，bot 走不到）。
  要做的话得先算清那一发会让收方多吃几个句柄。
- ⬜ **剩下 3 类武器**（会话 21 已经拿回 9 把，§72）：`BounceBullet`
  （角色 106 / 110 的 2 号，弹墙）、`RasTurret`（107 / 108 的 2 号，炮台）、
  `PlasmaCannon`（109 的 3 号，自己 `++` 句柄计数器）。
  都是**飞行**没模型，不是句柄问题。
