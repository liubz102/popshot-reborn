...
最后更新：2026-08-27（会话 18）

---

## 现在的位置

### ✅ 「别人看不见 bot 的子弹」**结案了**（§62 / D41，会话 18 实机验证通过）

**根因**：服务端把 bot 的**枪口**放在「落脚点上方 20 个单位、没有横向偏移」，
而真人客户端实测是**身前 43、脚上 57**。枪口落在**碰撞盒里** ⇒
弹体**第一次推进就撞掉**，位置一步没动过 ⇒ 屏幕上什么都没有。

**证据**（同一局、同一角色、同一把枪 `1000010`，`PROJ.` 逐帧日志）：

| | 被推进的帧数 | 走过的距离 |
|---|---|---|
| 真人自己的子弹 | 5~11 帧 | 每帧 129 单位，飞出屏幕 |
| bot 的子弹（修复前，几十发无例外）| **1 帧** | **0** |
| bot 的子弹（修复后）| **11 帧** | **1287 单位**，截图能看见曳光线 |

★ `/noboom`（一发爆炸都不发）**照样只有 1 帧** ⇒ 不是爆炸包杀的。
★ 用户三次说「和距离无关」，**他一直是对的**：枪口在自己身上，多远都一样。

**改了什么**（`server/bot.py`）：新增 `BOT_MUZZLE_HEIGHT = 57` /
`BOT_MUZZLE_FORWARD = 43` + `_muzzle()`（横向偏移跟着朝向翻）；
`BOT_AIM_HEIGHT` 20 → 57（枪口对枪口）。测试 `approach_far()` 跟着改
（跟随状态下拉不出「在飞」这个状态，先 `holding` 钉住再拉开）。

**教训（比结论值钱）**：五轮实机 + 三轮逆向都在问「弹体**长什么样**」，
没有一轮问「弹体**走没走**」。`PROJ+` 那份全字段快照只拍出生那一刻，
真正的判据是**第 2 帧还在不在**。

### ⏳ 还剩的两笔账（都归 M5，别和上面那条混）

1. **贴脸仍然没有行程**：bot 跟随时永远只落后 `BOT_FOLLOW_DISTANCE`（120），
   枪口再往前 43 ⇒ 枪口到目标最多 77 单位，初速 100 的枪连一个客户端
   tick（32 ms）都飞不满，走 `instant` 当场结算。
   ⇒ **「保持交战距离」是 M5 的第一件事**。
2. **爆炸时刻粗到一个 bot 帧**（~125 ms）：诊断那局瞄 400 单位、弹体飞了
   1287 单位才没，爆炸特效和弹体位置对不上。

### 🔴 用户给的硬事实（留着，验收时照这个看）

1. **弹道预测线 ≠ 子弹**：真人一进游戏就一直看得见**自己的**预测线（别人的看不见），
   随鼠标动；1/3 号武器白直线、2 号武器抛物线；开枪瞬间消失一下再回来。
2. **真正的子弹有模型、看得见在飞**。角色 2：1 号 = 双散弹枪（分散 3 条线）、
   2 号 = **手雷**（燃烧弹似的模型）、3 号 = **火箭弹**（火箭模型 + 末段追踪）。
...
| **不许再问「你看得见吗」** | 只换回一个布尔值，代价是用户真进游戏挨打；要查就自己跑（会话 17/18 已经把这条路打通了，见 sessions/2026-08-27-18.md）|
| **看到疑似目标物先找证伪观察** | §59 就是把预测线当子弹，一路顺着它编下去的 |

### ★ 会话 18 顺带钉死 / 推翻的（**§61**，从会话 17 的日志里挖出来的）

1. ★★★ **客户端没有本地预测** —— 真人自己开的枪也是等服务端把 `rpFire` 发回来
   （`发方 = 我的座位`）才造弹体，句柄步长 13 发全是 2（有本地那一份就该是 4）。
   ⇒ 真人的子弹和 bot 的子弹走的是**同一个函数、同一种包**。
2. ★★★ 同一把枪的并排快照，**弹体对象逐格一致**（连武器定义都是同一个指针）
   ⇒ 差别不在弹体的**内容**上，在它**动没动**。
3. ⛔ §61.3「直射枪本来就只活 1 帧」**已被 §62 推翻**（那是贴脸打中人的取样）。
4. ★ 线对象 `+0x20` 是**拖尾每个点存活多少毫秒**的逐武器参数，和这个 bug 无关。
5. ⚠ **工具口径**：`bslog` 一行约 2 ms，`PROJ.` 的时间戳**不能**当帧率用（§61.6）。

★ 仍然成立：收侧从分发到渲染**一处 owner / 座位分流都没有**（**§57**）；
`EffectMngr` 建线那一段也没有（§61.4）。
...
★★ **真正该修的是 bot 站位**（让它保持交战距离），归 **M5** —— 先等下面那条实机。
...
**会话 18 改了什么**（代码已落盘，**没有提交**）：

| 文件 | 改动 |
|---|---|
| `server/bot.py` | ★★★ **修根因**（§62 / D41）：新增 `BOT_MUZZLE_HEIGHT = 57` / `BOT_MUZZLE_FORWARD = 43` + `_muzzle(x, y, toward_x)`；`_fire_target()` / `_try_fire()` 一律从枪口算弹道、发包；`BOT_AIM_HEIGHT` 20 → 57 |
| `server/test_botsync.py` | `approach_far()` 改成「先 `holding` 钉住 bot、再让真人走远」（跟随状态下枪口到目标只剩 77 单位，拉不出「在飞」这个状态）；`BotPointBlankTests` 那条真飞用例跟着改，断言换成「到点时刻在未来」 |
| `hook/bshook.c` | ★ 探针加料：`FIRE>` 多打**射手角色的真实位置 + 发射点−角色**（就是这一行逮到的根因）；`PROJ+` 加 `绑定(+8c)`、三个特效对象 dump 加长到 20 dword；`PROJ.` 每帧顺带 dump 线 / 拖尾。**M3b 收口后删** |
| `hook/bin/` | 重编（`bshook.c` 零警告）|

**测试**：`C:\Python314\python.exe server\run_tests.py` → **1365 全绿**；
`runtime-win7\python\python.exe`（CPython 3.8）→ **同样全绿**。
**实机**：自己跑了三局（见上面「实机自助跑的路子」），修复前后的逐帧日志
和截图都在 `logs/bshook_20260827_14*.log`。
...
| M3b | bot **会开枪** | ✅ **完成**：打得中、换得了枪、**子弹看得见**（会话 12~18 实机通过）。★ 弹道模型（§47）、三个槽位全可用（D35）、语料口径的射程（§48）、**弹匣节奏**（D36）已落盘；★★ **「别人看不见 bot 的子弹」已修好**：枪口位置填错，弹体一出生就撞掉（**§62 / D41** 实机验证）|
| M4 | 地图地形数据 | ✅ **完成**（会话 06，实机核对通过）：174/174 提取、加载器 + 测试 + 打包钩子；值 1 = 单向平台已查实（§29） |
| M5 | bot AI（寻路 / 追敌 / 瞄准 / 难度） | 🔜 **下一个要做的**。★★ 第一件事仍然是 **保持交战距离**：跟随时枪口到目标只剩 77 单位，子弹连一个客户端 tick 都飞不满（§62.4）—— 现在它不再是「看不见子弹」的根因，但仍然是观感和爆炸对位的关键 |
...
⚠ **它不再是「看不见子弹」的根因** —— 那条已经在 §62 / D41 修掉了。
它现在要解决的是**观感**：跟随时枪口到目标只剩 77 单位，子弹连一个客户端
tick 都飞不满（`instant` 当场结算），而且爆炸特效和弹体位置对不上。

### 要做什么

1. `trail_point()` 只回放真人轨迹，真人不动时它必然贴身（D16 的固有缺陷）
   —— 得给 bot 一个**独立于轨迹的站位约束**：离最近的敌人不少于某个距离；
2. 退开要**踩着地面**退，不能穿墙 —— 用 M4 那份逐像素碰撞位图
   （`mapdata`，`is_solid` / `blocks_bullet` 两套口径，值 1 = 单向平台）；
3. 距离取多少：语料量出来的真人交战距离，**打中人的中位数 559**、
   p25=390、p75=722（§52 的表）—— 从 400~600 起步。

### 验完就删的诊断脚手架（D40 的收口清单，D41 沿用）
