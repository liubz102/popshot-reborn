...
最后更新：2026-08-28（会话 24）

---

## 现在的位置

### ⏳ **等用户实机验收**（会话 19 ~ 24 攒在一起）
...
| 「地上的燃烧持续伤害有时候 20 有时候 10，bot 的每次固定 10」 | 见下面「⚠ 这一条只做到一半」 | **§79** 顺带（墙宽了 3 倍，重叠的机会跟着大了）|
...
语料能钉死的只有两件事：**同一个人两次挨烧至少隔 20 个 tick**、
**一道墙最多烧 `寿命/20` 次**（§78，会话 24 复量 181 次火烧，
「同一发心跳里挨两次」**一次都没有**）。所以现在的模型仍然是
「每道墙对每个人 20 tick 一次、每次 10 点」。

全语料里确实有 **9 次**「同一发心跳挨两发 10 点」，而且**每次都来自两个
不同的源句柄** —— 也就是**两道火墙重叠**（或者两团火同 tick 结算）。
新模型把墙从 ±45 拓宽到 ±120，重叠的机会大了不少，所以 20 点应该会出现。
⏳ **实机看下来要是还是恒 10，告诉我**，我按「每团火各记各的账」重量一遍
（那会让一道墙一次烧 20~30，得先有证据）。
