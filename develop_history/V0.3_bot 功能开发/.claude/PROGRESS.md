...
最后更新：2026-08-27（会话 20）

---

## 现在的位置

### ⏳ **等用户实机验收**（会话 19 + 会话 20 攒在一起）
...
7. ★ **`/gun` 换枪时会发现有些槽位没了**（角色 3/106/110 只剩 1 号枪）——
   那是 §70 故意收紧的：那几把在收方会多吃弹体句柄，用了就静默地
   「子弹照飞、一滴血不掉」。**不是坏了。**
...
- ⬜ **地面燃烧（火墙）**：`rpSetOnFire`（`0x0005`）的布局已经逆完
  （§69 / packet_api §5.4d），但唯一会用它的 `FlamingBottle` 被 §70 剔出
  `usable` 了 —— 要做火墙得先解决那一类的句柄记账。
- ⬜ **把被剔掉的 15 把武器拿回来**（分裂弹 / 火墙 / 炮台 / 反弹弹 /
  定时炸弹，§70）：得逐类逆清「收方多创建几个对象」和「弹道长什么样」两件事。
