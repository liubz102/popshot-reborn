...
* **打怪 / 打 boss**：★★ **怪的位置一个包都不广播**（§125，380 份语料里心跳的
  `count` 恒为 1）。服务端只能在「怪开了一枪」和「有人打中了怪」这两个瞬间
  瞄一眼，`bot.mob_sightings()` 就是这本账，1 秒过期。朝新鲜目击点开枪，
  弹体扫中**带句柄**的目击点就按那个句柄发 `rpExplode`（收方照着扣血）。
  ⚠ **打偏是常态**，那是信息量的极限不是 bug。
...
10. ★★ **闯关模式**：bot 现在**自己走路**（以前是回放你的轨迹）。
    预期：跟得上、不甩你太远、会躲怪的子弹、会朝怪打。
    ⚠ 最该盯的两件：① **会不会卡住 / 掉队**（自己走就有这个风险，
    以前回放轨迹天然不会）；② **打怪准不准** —— 它只在「怪开枪」和
    「有人打中怪」那两个瞬间知道怪在哪，所以**经常打偏是预期**，
    但如果「明明没碰到怪却掉血」就说一声（`MOB_HIT_RADIUS` 该调小）。
