...
| 打包：16 卷重写、五份产物重生成、`BUILD.ver` / `build-ver.config` 抬到 **0.4.2**、`manifest-hook.json` 加 V0.4.2 | ✅ |
...
1. **等用户选撞色**（A 黑曜 / B 宝蓝 / C 银白，可每把不同；火墙留不留原色）→
   `C:\Python314\python.exe tools\goldwp\build.py --variant X` → `tools\build-pack.bat`（幂等，只重写变了的卷）。
   要每把不同的话在 `spec.WEAPONS` 旁加一张 `{物品id: 变体}` 表，`build.py` 按表取。
2. **实机 V26~V30**（下表）。V27 前先在管理页「金币 / 经验」页把一把自定义武器发给自己（出厂不上架）。
3. 发版：客户端包 + 服务端包都要重打（资源 + `bshook.dll` + 服务端代码）。**版本号 0.4.2 是我抬的工作版本**，
   发版前用户拍板；`WEAPON_TABLE_MIN_VERSION` 跟着改。⚠ 线上 V0.4.1 客户端连本地 0.4.2 服务端会被门禁拒。
...
⚠ 一条小尾巴：`hook/bshook.c` 最后一次改动（日志去重键加上「写入条数」，只影响日志）**还没编进
`hook/bin/bshook.dll`** —— 当时用户的客户端正开着，链接器打不开 DLL（`LNK1104`）。游戏关掉后跑一次
`hook\build.bat` 即可（`build-ver.config` 已是 0.4.2，只会刷新 V0.4.2 那一条清单）。现在的 DLL / 清单 /
运行中的客户端三者一致，功能上没有差别。
...
| V29 | 看提示框：商店（若上架）/ 仓库 | 自定义武器第一行「仅显示PVP属性，PVE属性请看GM管理页：」，数值是 PVP 那套；改说明文后商店提示框即时、仓库提示框重登后更新 | ⏳ |
...
9. 版本 0.4.2 只是工作版本；`manifest-hook.json` 里 V0.4.1 的哈希 `9b26e1f5…` 是线上那份，**别让 `hook\build.bat` 刷掉它**
   （生成器只刷 `build-ver.config` 里的版本，抬版本后再编译就不会碰老条目）。
