...
class Room:
    ...
    def __init__(self, room_id, host_conn, *, title="", map_name="",
        ...
        #: ★★ 本房间的 bot AI 策略（V0.3 M5）。这两格属于**房间**，不是
        ...
        self.epoch_value = 0
        #: ★★★ 本房间那条 **32 ms 战斗循环**（`gameserver.RoomLoop`），V0.3 D106。
        #: 和 `battle` / `quest` 一样：`lobby.py` **只管模型不碰协议**，
        #: 这里只留一个空槽，建和用都在 `gameserver.py` 里。
        self.sim = None
        #: ★★★ 这一局**模拟状态**的统一锁（D106）。
        #:
        #: 以前 bot 的帧挂在真人的转发路径上（D17），房里两个真人就是**两条
        #: 线程**同时推同一批弹体 —— 那个竞态一直在，只是没人踩响。现在帧由
        #: 房间自己那条线程走，网络线程只在这把锁里**记输入**（轨迹点、怪的
        #: 位置、挨打的击退），于是「每个 tick 开始前，已经收到的输入一定可见」
        #: 是结构性成立的，不靠时序运气。
        #:
        #: 锁序 **`Conn.peer_lock` → `Room.sim_lock` → `Conn.send_lock`**，
        #: 反过来拿会死锁：持 `sim_lock` 时不许去拿任何 `peer_lock`。
        #: 用 RLock —— 一个 tick 里会层层调进 `bot.py`，那边还会再进来一次。
