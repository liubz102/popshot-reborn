...
#:
#: ★ 难度按**效果强弱**分四档，不按「卡片好不好拿」排 —— 好不好拿是运营
#:   在「称号卡片」那一页随时能改的，效果是客户端写死的。
#:
#: ★★ 下面这些数字是**用户 2026-09-14 在管理页上逐条调完之后回填的**（D116）：
#:   四档的**金币**没动，动的全是**张数** —— 武器称号一律 20 张、成就称号
#:   10 ~ 166 张。
#: ★★ `560004 [幸运幸存者]` 是 2026-09-25 用户改的 **166 张 + 6666 金币**：
#:   它原先被当成全表最强（「受击 50% 免伤」）、定成全表最贵的 666 张 +
#:   111666 金币；X_Mod §90 查实它只在「剩余 HP < 15 且这一下会被打倒」
#:   时才掷那 50%（`SPECIAL_EFFECT_BY_ID`）⇒ 降了价。
...
TITLE = {
    # ---- A：残血挨致命伤时 50% 免伤 ----
    560004: (6666, ((60004, 166),)),            # [幸运幸存者] ← 幸运卡片
    # ---- B 强 ----
