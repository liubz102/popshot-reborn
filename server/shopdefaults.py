...
NAME_BY_ID = {
    ...
    # ★ 三张改过名的（用户 2026-09-13 拍板）：2007 新浪那份译名（厄运 / 乌龙 /
    #   信心）和韩文原意对不上，而卡片名**就是获得条件**，对不上会让玩家看着
    #   「信心卡片 ← 自杀 3 次」一脸茫然。按原意改：
    #     60005 빈대（蹭吃蹭喝的人）→ 蹭分卡片 ← 零击杀却赢了 → [手下留情]
    #     60007 팀킬쟁이（杀队友的）→ 误伤卡片 ← 误伤队友    → [队内间谍]
    #     60008 제풀쟁이（自己把自己搞死的）→ 自爆卡片 ← 自杀 → [自爆达人]
    #   ⚠ **线上那份 `items.json` 不会跟着改**（`backfill_defaults` 只增不改，
    #     铁律 11）—— 要改得去管理页「物品库」手动改那三条。
    60001: "完美卡片", 60002: "格斗卡片", 60003: "射手卡片", 60004: "幸运卡片",
    60005: "蹭分卡片", 60006: "红心卡片", 60007: "误伤卡片", 60008: "自爆卡片",
...
#: 称号配方（V0.3商店 · 称号卡片）：`{称号 id: (金币, ((卡片 id, 张数),…))}`。
#:
#: 对应关系照原版：成就卡 `60001`~`60008` ↔ 称号 `560001`~`560008`，
#: 武器卡 `110001` 等 ↔ 称号 `610001` 等（卡片 id = `weapon.ini` 的 ROH，§37）。
#: **一条配方只用它自己那张卡**，没有「凑材料」这回事 —— 称号是成就的兑换物。
#:
#: ★ 难度分四档，锚点是 **`560004 [幸运幸存者]`**（它是全表最强的一个：
#:   受到伤害时 50% 概率完全免疫，`SPECIAL_EFFECT_BY_ID`）。
#:   其余按**效果强弱**往下排，不按「卡片好不好拿」排 —— 好不好拿是运营
#:   在「称号卡片」那一页随时能改的，效果是客户端写死的。
#:
#: ★★ 下面这些数字是**用户 2026-09-14 在管理页上逐条调完之后回填的**（D116）：
#:   四档的**金币**没动，动的全是**张数** —— 武器称号一律 20 张、成就称号
#:   10 ~ 666 张，锚点那条从「500 张 + 100000 金币」改成
#:   **666 张 + 111666 金币**（★ 全表最贵的一条，比第二贵的 `560006` 还多
#:   六倍半 —— 它换的是唯一那个「50% 概率完全免伤」）。
#:   ⚠ 张数和「称号卡片」那一页的获得条件是**一对**（`CARD_RULES` 同一天
#:   一起改过）：单看一边看不出难度，要两边一起读。
#:
#: ★ `560010` / `560011` / `560012` 是**活动称号**，原版就没有对应的卡片
#:   （`[전국일등]` / `[자유의 날개를 펴고]` / `[2007새내기]`）⇒ **故意不给配方**，
#:   运营想发就用管理页的「批量发送奖励」。这不是漏了。
#:
#: ⚠ `560002 [格斗达人]` 的加成要求 `GetPvpMode() == 2`（격투 모드），
#:   而**中国区客户端选不到那个模式**（`shopcfg.PVP_MODE_ZH` 只有生存和夺分）
#:   ⇒ 穿上去防御 +0。照原版保留、照样上架（铁律 12：不发明原版没有的玩法），
#:   代价由说明文字兜着（`shopcfg.item_desc_zh` 会把这句写进提示框）。
TITLE = {
    # ---- A 最强：50% 概率完全免伤 ----
    560004: (111666, ((60004, 666),)),          # [幸运幸存者] ← 幸运卡片
    # ---- B 强 ----
    560005: (30000, ((60005, 50),)),            # [手下留情] 伤害翻倍（45 点额度）
    560006: (30000, ((60006, 100),)),           # [红心达人] 捡心 15% 全队回血
    610001: (40000, ((110001, 20),)),           # [左轮手枪高手] 本系攻击 +15%
    620001: (40000, ((120001, 20),)),           # [复古短枪高手] +15%
    630001: (40000, ((130001, 20),)),           # [重机枪高手] +15%
    # ---- C 中 ----
    610002: (20000, ((110002, 20),)),           # [苹果弹高手] +5%
    620002: (20000, ((120002, 20),)),           # [火焰弹高手] +5%
    630002: (20000, ((130002, 20),)),           # [榴弹发射器高手] +5%
    610003: (15000, ((110003, 20),)),           # [狙击枪高手] +3%
    620003: (15000, ((120003, 20),)),           # [华尔兹加农炮高手] +3%
    630003: (15000, ((130003, 20),)),           # [火箭炮高手] +3%
    # ---- D 弱 / 纯荣誉 ----
    #   ★ `560001` 的加成是「攻击 +5、生命 **−10%**」—— 净收益很可能是负的，
    #     所以它便宜，贵的是那张卡本身（零死亡赢一局 / 通一关才给一张）。
    560001: (5000, ((60001, 50),)),             # [完美胜利者]
    560002: (5000, ((60002, 50),)),             # [格斗达人] ⚠ 加成不会触发，见上
    560003: (5000, ((60003, 50),)),             # [射击达人] 生存 / 夺分防御 +2
    560007: (3000, ((60007, 10),)),             # [队内间谍] 对队友伤害 −5%
    560008: (3000, ((60008, 20),)),             # [自爆达人] 自伤 −5%
}
...
#:
#:   무적승리자 无敌胜利者 → 零死亡获胜        빈대 蹭客       → 零击杀却赢了
#:   파이터 斗士           → 近身打（突击技）   하트마니아      → 捡心
#:   슈터 射手             → 命中率            팀킬쟁이 队杀鬼 → 误伤队友
#:   제풀쟁이 自爆鬼       → 自杀              리볼버고수 等   → 用那把枪打死人
#:
#: ★ **一局就是一次机会，不掷骰子** —— 卡片是成就不是掉落（D44a 当初砍掉
#:   「按概率掉卡片」就是这个理由）。所以这张表里没有「概率」这一格。
#: ★ `60002 斗士` 数的是**突击技命中**不是发动（用户 2026-09-13）：
#:   发动次数空房间连点就达标，不算成就。
#: ★ `60003 射手` 那条**命中率**后面必须跟一条「开枪数」：开一枪中一枪
#:   也是 100%，少了它命中率那一格没有意义。
#: ★ 武器卡**只给对战**：闯关一局里怪成打，「用左轮打死人」等于「参战就给」，
#:   那张卡就没有含义了。运营想放宽在管理页把模式改成不限即可。
#:
#: ★★ 具体的**阈值 / 统计范围 / 模式**是用户 2026-09-14 在管理页上逐条调完
#:   之后回填的（D116），和 `TITLE` 那张表同一天改的，要一起读。这一轮的形状：
#:   一半的卡片从「本局达标」挪到**累计达标**（攒够就归零，D111）或者补了一条
#:   「或者累计够了也给」的兜底；`60001` / `60006` / `60007` / `60008`
#:   不限模式（闯关也发）；`60003` 改成看命中率。
CARD_RULES = {
    # 卡片:   (模式,  [达成条件…])
    60001:  (None,   [card_cond("deaths", shopcfg.CARD_OP_EQ, 0), _WON]),
    60002:  ("pvp",  [card_cond("dash_hits", threshold=10,
                                scope=shopcfg.CARD_SCOPE_TOTAL)]),
    60003:  ("pvp",  [card_cond("accuracy", threshold=60),
                      card_cond("shots", threshold=10,
                                join=shopcfg.CARD_JOIN_AND)]),
    60004:  ("pvp",  [card_cond("guards", threshold=36), _WON,
                      card_cond("guards", threshold=166,
                                scope=shopcfg.CARD_SCOPE_TOTAL,
                                join=shopcfg.CARD_JOIN_OR)]),
    60005:  ("pvp",  [card_cond("kills", shopcfg.CARD_OP_EQ, 0), _WON]),
    60006:  (None,   [card_cond("hearts", threshold=3)]),
    60007:  (None,   [card_cond("team_kills", threshold=50,
                                scope=shopcfg.CARD_SCOPE_TOTAL)]),
    60008:  (None,   [card_cond("suicides", threshold=3),
                      card_cond("suicides", threshold=50,
                                scope=shopcfg.CARD_SCOPE_TOTAL,
                                join=shopcfg.CARD_JOIN_OR)]),
}
#: 武器卡：用那一族武器**累计**打死 N 个人（攒够就归零，D111）。
#: ★ 三档**一样难**（用户 2026-09-14 拉平的，原来是按称号加成强弱 5 / 4 / 3
#:   分三档）—— 梯度现在只由 `TITLE` 那边的金币承担。
#: ★ 指标就是普通的「击杀数」，「用哪把枪」由条件里那一格 `weapon` 回答。
_WEAPON_CARD_KILLS = 50
for _roh in shopcfg.WEAPON_CARDS:
    CARD_RULES[_roh] = ("pvp", [card_cond(
        "kills", threshold=_WEAPON_CARD_KILLS, weapon=_roh,
        scope=shopcfg.CARD_SCOPE_TOTAL)])


def build_cards():
    """默认 `cards.json` 的 `rules` 列表，按卡片 id 排序。

    ★ 物品表里查不到的卡片整条跳过（测试用的小表），别生成一条校验不过的规则。
    """
    rules = []
...
def build_all():
    ...
    return {
        ...
        "drops": build_drops(),
        #: ★ 称号卡片的获得规则（V0.3商店）。它和物品表无关（`build_cards()`
        #:   只按 id 过滤），放进来是为了让 `problems()` 那道
        #:   「配方的材料拿得到吗」的自检看得见卡片这条来路。
        "cards": default_cards(),
...
def default_cards():
    """默认 `cards.json`。★ 同样不走 `build_all()`：17 条规则是手写的设计表。"""
    return {"format": FORMAT, "rules": build_cards()}
...
def problems(built):
    ...
    # ★ 「拿得到」有两条来路：材料掉落表，和**称号卡片的获得规则**
    #   （V0.3商店）—— 卡片不走掉落表（它是成就，不是概率掉落，D44a），
    #   所以这里要把 `cards.json` 那一份也算进来，不然 17 条称号配方每条都喊
    #   「材料没有掉落规则」，这道自检就被喊废了。
    cards_ok = shopcfg.validate_cards(built["cards"])
    droppable = {r["material"] for r in drops_ok}
    droppable |= {r["card"] for r in cards_ok if r.get("listed")}
    for recipe in recipes_ok:
        total = sum(m["count"] for m in recipe["materials"])
        # ★ 称号不受这条限制：它换的是**成就卡片**，「500 张幸运卡片」正是
        #   用户要的难度（30 这个数是给「一件装备要几块矿」定的）。
        if total > 30 and shopdata.kind(recipe["result"]) != "title":
