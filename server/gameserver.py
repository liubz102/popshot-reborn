...
MAGAZINE_STATUS = {
    ...
    10500: (10, 3, 1.0, 1.0),     # BulletPoison 毒弹
}
...
#: ★★★★★ **腾空**的 bot 心跳隔几个 tick（V0.3 §191 / D150）。
#:
...
#:   它和上面那个 4 是同一性质的常量，不是铁律 10 禁的那种阈值。
AIR_HEARTBEAT_TICKS = 1
...
class Conn:
    ...
    def feed(self, data):
        ...
        if not self.got_version:
            ...
            #   客户端会拉起更新器自己换回正版那份。
            #   ★ 清单里没有这个版本：比清单里最老的还老 -> 交给版本门禁（那一版
            #   发布时还没有这项功能）；否则一律拒（服务端保证先于客户端更新，
            #   D85 二轮）。服务端自己的版本不在清单里时整项自动关掉（安全阀，
            #   否则被拒的客户端下回来的还是同一版 -> 死循环）。
