...
DEFAULTS = {
    ...
    "proxy_username": "",
    "proxy_password": "",
    # 双击 A/D 出近身攻击的判定窗口（毫秒）。**只在客户端包里有意义**，
    # 服务端读到它也不会做任何事。250 = 原版；见 FINDINGS §183 / D105。
    "double_tap_ms": 500,
}
...
# ---------------------------------------------------------------------------
# 双击 A 或 D 出近身攻击的【判定窗口】，单位毫秒。原版是 250。
#
# 为什么默认改成 500：出一次近身攻击之后角色有 893 毫秒的动作锁定，这期间你按
# 的每一下客户端**全部作废**（连「这次点按」都不记）。锁一解开，原版要求你再
# 干干净净地点两下、而且两下相隔不到 250 毫秒才算数 —— 连按时点按几乎全落在
# 锁定期里，于是「按得再快也发不出」。窗口放宽到 500 毫秒之后，锁定一结束你的
# 下一下就能配对成功。
#
# ★ 它【不改】攻击频率上限：893 毫秒的动作锁定和 600 毫秒的冷却都原样保留，
#   近身攻击仍然是大约一秒一次。改的只是「你的点按算不算数」。
#
#   double_tap_ms = 250     <- 完全回到原版行为
#   double_tap_ms = 500     <- 默认
#   数值越大越容易出招，但「原地小幅调整位置连点两下」也更容易误触发。
#
# 这一项只对客户端有意义，服务端读到它不会做任何事。
# ---------------------------------------------------------------------------
double_tap_ms = 500
