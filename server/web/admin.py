...
def _afk_places():
    """「挂机中」= 进图了、但看不出他在玩。判据有三条（`gameserver.conn_is_afk()`）：

    1. 多人局：连续超过 `AFK_AFTER_S` 秒没碰过方向键之外的操作；
    2. 单人局：连续超过 `AFK_SOLO_AFTER_S` 秒没打中 / 没捡到 / 没得分；
    3. ★ **客户端报上来的在场证据**（2026-09-21 接上，§62 / D53）——
       窗口不在前台 / 这台机器前面没人 / 键盘鼠标少了哪一半（只有鼠标 /
       只有键盘 / 两边都没动），稳住 `PRESENCE_AFK_AFTER_S` 秒就算。
       **单人任务房里只有这一条管用**：那种房间服务端一发同步包都收不到，
       而挂机的人恰恰全打单人局。
