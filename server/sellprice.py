...
def quote(item_id, prices, shop_table, recipe_by_result):
    ...
    if not shopdata.ownable(item_id):
        ...
        return blank
    # ⚠ **称号照样卖得掉**（用户 2026-09-13 拍板，**推翻 D103**）：
    #   一度给它加过一条「荣誉不是商品」的闸，用户明确要求不要擅自加限制。
    #   ⇒ 它走的是下面第三条分支（有配方 ⇒ `配方花费 × 百分比`），
    #     `[幸运幸存者]` 的出厂配方是 6666 金币 ⇒ 按出厂的 95% 卖 6332。
    #   ★ 嫌高就在管理页「⚙ 卖出价格设置」里把装备百分比调下来，
    #     那本来就是运营的旋钮（D94 已经把上限收到 100%）。
    name = material_class(item_id)
    if name is not None:
