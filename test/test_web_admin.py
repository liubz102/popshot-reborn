...
#: 等审计日志那一行落地的**保险丝**（秒）。
#:
#: ★★ 它**不是判据**。判据是 `_CapturedLog` 里那个条件变量 —— 行一 append
#: 就 `notify_all`，正常情况下一毫秒都用不上。它只防一件事：真出毛病时
#: （服务端压根没写这一行）别让测试永久挂住，而且兜到了要**说清楚**。
#: 写法照 `testsupport.OUTBOX_FUSE_S`（铁律 10：固定时长只能当保险丝，
#: 不能当判据）。
AUDIT_FUSE_S = 60.0


class _CapturedLog(list):
    """`capture_log()` 拦下来的审计日志行。**跨线程**：append 的是 HTTP 处理线程。

    ★★ 为什么不能直接 `assertEqual(1, len(lines))`：日志下载那一路
      （`server/web/admin.py` 的 `_admin_logs_download`）是**先把 chunked 结束块
      写进 socket、再写审计日志**的，而 `wfile` 不带缓冲（`wbufsize = 0`）——
      客户端 `read()` 返回时服务端那一行还没落地。直接断言等于赌服务端线程
      跑得比客户端快：3.14 上赌赢，Win7 的 3.8 并行跑全量时赌输，
      报出来是「审计日志一行都没有」（`1 != 0 : []`，2026-09-18 实测）。
    ⇒ 要断言「写了几行」的用例先 `wait_for()`：等的是**那一行本身**，
      不是固定的秒数。
    """
    ...
    def wait_for(self, case, count=1):
        """等到攒够 `count` 行；等不到就**当场失败、并说清是超时**。

        返回自己，方便串起来写。
        """
        with self._cond:
            if not self._cond.wait_for(lambda: len(self) >= count, AUDIT_FUSE_S):
                case.fail("审计日志在 %g 秒内没等到第 %d 行（只等到 %d 行：%r）"
                          "—— 这是时序或死锁，**不是**「服务端没写审计日志」，"
                          "别照着这条错话去查 server/web/admin.py。"
                          % (AUDIT_FUSE_S, count, len(self), list(self)))
        return self
...
class _AdminCase(unittest.TestCase):
    ...
    def capture_log(self):
        ...
        ★ 回的是 `_CapturedLog`（`list` 的子类，用法不变）：写这一行的可能是
          另一个线程，要断言「写了几行」得先 `wait_for()`，见那个类的说明。
        """
        lines = _CapturedLog()
...
class AdminLogsApiTests(_AdminCase):
    ...
    def test_the_audit_log_says_who_took_what(self):
        ...
        # ★ 事件驱动：等的是那一行审计日志本身 —— 服务端是**发完结束块才写**
        #   它的，`download()` 返回时它还不一定落地（见 `_CapturedLog`）。
        lines.wait_for(self)
