...
def load_generator():
    ...
    # ★★ 挂在**最后面**，不是 `insert(0, …)`：`tools/` 和 `server/` 有四个
    #    同名模块（`mapdata` / `chrprops` / `shopdata` / `weapondata`），排到
    #    前面去的话，同一个 worker 进程里跑在后面的分片 `import mapdata` 会
    #    拿到离线提取器那一份（见 `test_chrprops.load_tool` 的说明）。
