...
"""跑全部服务端测试。
    ...
    python test/run_tests.py              # 默认**并行**，按 CPU 数开子进程
    python test/run_tests.py -j1          # 串行，和并行化之前逐字一致
    python test/run_tests.py test_shop    # 只跑这几个模块（也能写 模块.类）
...
`ModuleNotFoundError`。这里显式把 `test/`（测试自己）和 `server/`（被测代码）
都放进 `sys.path` 再跑，系统 Python 和便携 Python 就用同一条命令了。

★ 测试代码和测试数据都在 `test/` 下（`test/data/<夹具>/`），被测代码在
`server/`；两个目录是平级的，所以 `os.path.dirname(HERE)` 就是仓库根。
...
SERVER = os.path.join(ROOT, "server")                   # 被测代码
for _path in (SERVER, HERE):
...
def all_modules():
    ...
    ★★ 以前这里是一张手写的表（`MODULES`），加了新测试模块要记得往里补一笔。
      这一笔**至少漏过三次**：`test_pkn` / `test_packdirs` / `test_questrecord`
      进了仓库却从没被跑过（2026-09-18 查明，补跑 64 条，全绿）。
      漏登记的症状**不是报错，是「全量绿」里静悄悄少几十条用例** ——
      2026-09-17 就吃过亏：pkn 金样被当成「无用的测试数据」删掉，
      守着它的 `test_pkn` 不在名单里，谁也没发现。
      ⇒ 名单只留**一个**来源（目录本身），这一类漏登记就不可能再发生。

    ★ 这里的顺序无所谓：真正的发片顺序来自上一轮量出来的
      `logs/.test_shards.json`（见文件头）。排序只为**确定**，换台机器一样。

    ★ 一个都没扫到 = 出事了，不是「没有测试要跑」，要红着退出，
      绝不能报一句 `Ran 0 tests / OK`。
    """
    names = sorted(entry[:-len(".py")] for entry in os.listdir(HERE)
                   if entry.startswith("test_") and entry.endswith(".py"))
    if not names:
        raise SystemExit("%s 里一个 test_*.py 都没有 —— 测试代码全在这个目录下，"
                         "跑之前先确认仓库是完整的。" % HERE)
    return names
...
_SHARD_TIMES = os.path.join(ROOT, "logs", ".test_shards.json")
...
def _run_parallel(names, jobs, complete):
    ...
    def pump():
        child = subprocess.Popen(
            ...
            # ★ 工作目录**照旧是 `server/`**（测试代码搬进 test/ 之前就是这个值）：
            #   用例本来就不该依赖 cwd（串行跑的时候 cwd 是用户敲命令的地方），
            #   但没必要借着搬家顺手换掉它 —— 换了出问题，查的人会往搬家上想。
            cwd=SERVER)
        try:
            while True:
                try:
