/* --------------------------------------------------------------------------
   ...
   每个来源的「速度」= **峰值秒速**（用户 2026-09-07 第四条）：探针按
   SPEED_BUCKET_MS 一格记字节，速度 = 测量段内任意连续 1 秒（滑动）收到的
   最多字节；不足 1 秒就收完的小文件按整段平均。
   代理的用法是直接拼在原地址前面：<代理>/https://github.com/...。

   测速本身（WinHTTP）由调用方以 speed_measure_fn 注入 —— 编排逻辑因此
   不碰网络（只用 net_http.h 里的 NetTrace 数据类型），selftest 用假测速函数
   把选源规则钉住。
   -------------------------------------------------------------------------- */
...
/* 三个数都是用户定的（2026-09-07，第三条把测速改成 5 个一组、每组 10 秒；
   manifest 兜底保持 5 秒）。界面文字、selftest、e2e 都按这几个宏走，改这里就够。 */
#define SPEED_WINDOW_MS   10000             /* 每个来源测 10 秒（含建连），各阶段超时也是它 */
#define SPEED_GROUP       5                 /* 代理每 5 个一组并行 */
#define SPEED_GOOD_BPS    (1ULL << 20)      /* 达标线：> 1 MiB/s（界面显示的也是 MiB/s） */
#define SPEED_BUCKET_MS   100               /* 峰值秒速的分格：1 秒 = 10 格滑动 */
...
typedef struct SpeedSample {
    ...
    NetTrace trace;               /* 分格字节记录；nbuckets = 0 表示没记（假探针） */
} SpeedSample;
...
/* 比较用的速度 = 峰值秒速（B/s）：测量段内任意连续 1 秒收到的最多字节
   （按 trace 的格滑动）；不足 1 秒收完、或没有分格记录时退回整段平均。 */
...
#define MANIFEST_ATTEMPT_MS 5000            /* 用户明示：测速改 10 秒后这里仍是 5 秒 */
