...
- **合成**：打关卡掉材料，在原版合成界面用材料合装备，139 条配方（含 17 条称号）
- **材料掉落**：结算界面「合成材料」栏显示材料掉落；闯关模式每关每档固定掉两样，
  对战模式按概率掉
- **仓库**：买到 / 合成出来的装备可以穿，穿上之后伤害、生命、防御在游戏里会相应改变
- **装备卖出**：不要的东西可以换回金币 —— **原版没有这个功能，所以它做在 GM 管理网页上**，
  玩家用自己的游戏账号登进去就能卖自己的东西。合成得来的装备卖掉会**把配方材料原样退回仓库**
- **GM 管理网页**：可以管理商店货架 / 合成配方 / 调整掉落材料 / 金币经验获得量 /
  设置卖出价格 / 修改玩家仓库 / 批量发奖励到玩家的礼物盒。
  - **点击注册页标题右边「⚙ GM 管理页 →」**，或者在注册页的 URL 后面加 `/admin` 都可进入。初始管理员账户密码 `admin` / `Admin123`，上线后应**尽快修改默认密码**。
  - **普通玩家用游戏账号也能登录**，看得到物品 / 商店 / 配方 / 掉落 / 金币经验这几页
    （**只能看不能改**，当资料站用），另外还有一页**「装备卖出」是他自己能用的**
    —— 卖的永远是他自己那个号的东西。
- **数据管理**：每天定时备份 `server/data/*.json`，可手动备份、可一键回滚；
  服务端日志和玩家客户端传上来的崩溃包可以在管理页里**直接打成 zip 下载**，不用远程登录服务器
- 详见下面「商店 · 合成 · 装备」和「GM 管理网页」两节

称号卡片也已完成：

- **卡片**：17 张 —— 8 张成就卡（零死亡获胜、零击杀获胜、格挡、命中率、捡心、误伤、
  自爆、突击技命中）+ 9 张武器卡（用特定武器累计击杀）。打完一局按条件判定，
  结算界面单独一栏显示获得卡片
- **称号**：卡片在合成界面的「称号」标签里合成；20 个称号，17 个有配方，
  另外 3 个是原版活动称号，只能由 GM 发放
- 称号穿上后**头顶显示中文**，属性加成可生效
- 获得条件、张数、金币都在 GM 管理网页的「**称号卡片掉落**」页里改
- 详见下面「称号卡片」一节

原版加密 pkn 资源包替换成自研的可编辑加密包：

- 原版把地图、模型、贴图、`.ini` 全锁在加密的 `Pack\*.pkn` 里，无法自己修改内容。
  现在这条链已改为：`Pack_develop`（明文，想改什么改什么）→ `build-pack` →
  `Pack_publish`（原版加密格式，客户端实际读的），详见下面「改资源」一节
- 顺带修好了原版中文版的**翻译错误和漏翻**：20 个称号、6 个商城角色名、地图主题与
  剧情简介、晋级任务界面、15 条战斗提示等，改错 7 处、补译 58 条
...
139 条配方，一条最多用 4 种材料（合成界面就 4 个槽）：
上衣 30 · 头脸 24 · 手套 20 · 下装 19 · 鞋 16 · **戒指 6** · **宠物 7** · **称号 17**。
...
打得好给**卡片** → 卡片在合成界面的「称号」标签里合成**称号** → 穿上身头顶显示中文称号、
属性跟着变。

### 卡片怎么来

**根据成就发放**：一局打完按条件判定，够了就给，显示在结算界面的
「称号卡片」栏。具体获得条件可以在 GM 管理页修改的“称号卡片掉落”页修改。
初期默认值如下：

| 类别 | 张数 | 默认条件 |
|---|---:|---|
| 成就卡 | 8 | 完美（零死亡获胜）· 蹭分（零击杀获胜）· 幸运（格挡）· 射手（命中率）· 红心（捡心）· 格斗（突击技命中）· 误伤（打到队友）· 自爆（自杀）|
| 武器卡 | 9 | 用那一族武器**累计**打死 50 个人，攒够就归零 |

- 部分条件按**累计**算（攒够归零），部分只看本局；成就卡里 4 张不限模式（闯关也发），
  其余只在对战判定。
- ⚠ **商城角色自带的枪基本没有武器族标记** —— 用商城角色打，武器卡拿不到、武器称号的
  加成也不触发。**原版就这样**。唯一的例外是**派克**：它的 2 / 3 号枪算「榴弹发射器」
  和「火箭炮」，那两张卡和那两个称号照常有效。


### 称号

20 个称号，17 个有配方（`[完美胜利者]` `[幸运幸存者]` `[左轮手枪高手]` …），
另外 3 个是原版活动称号，原版就没有对应的卡片，**默认值没有设置配方** ——
运营想发用管理页的「批量发送奖励」。

- ⚠ `[格斗达人]` 的加成要求原版的「格斗」对战模式，而**中国区客户端根本选不到那个模式**，
  所以它穿上去没有实际效果。

条件、阈值、配方张数和金币都在 GM 管理网页的「**称号卡片掉落**」和「**合成配方**」两页里改。
改完已经在线的人要**重登**才看得到新的提示框文字（客户端会缓存物品定义）。

### 查自己的卡片进度

GM 管理网页的「**称号卡片掉落**」页，工具条最右有一颗「**查看本人达成进度**」——
**玩家用自己的游戏账号登进去就能点**，看的永远是自己那个号：每张卡片写着已经拿了几张，
按**累计**算的条件带一条「现在 / 需要」的进度条，只看**本局**的条件则标「每局游戏内计算」。
...
| 身份 | 拿什么登 | 能干什么 |
|---|---|---|
| 系统管理员 | 管理员账号（出厂 `admin` / `Admin123`）| 全部十页，全部能改 |
| 运营 | 管理员账号 | 前六页 + 装备卖出都能改；「玩家仓库」**能查人、能发奖励，但改不了别人的仓库** |
| **普通玩家** | **游戏里那套账号密码** | 前六页**只能看**（输入框全是锁住的，没有添加 / 保存 / 删除）；**「装备卖出」这一页他自己能用** —— 卖的永远是他自己那个号的东西 |

普通玩家这一档**不用开通**，注册过游戏账号就自动有；给谁开修改权限，在
「管理员账号」页点**「＋ 指定玩家为管理员」**，在弹出的玩家列表里挑人
（他以后就用游戏里那套账号密码登管理页）。
...
| 称号卡片掉落 | 17 张卡片各自的获得条件：哪个模式、看哪个指标、本局还是累计、到多少给。★ 工具条上的**「查看本人达成进度」三档身份都能用** —— 看自己那个游戏账号离每张卡还差多少 | 系统管理员 · 运营 · 玩家（只读，**进度看得到**）|
| 金币 / 经验 | 打完一局给多少金币和经验 —— 对战按 模式 × 个人/组队，闯关按 关卡 × 难度，输赢分开 | 系统管理员 · 运营 · 玩家（只读）|
| **装备卖出** | **卖自己账号里的东西换金币**（原版没有这个功能）；管理员还能在这一页设置各类东西的卖出价 | 三档都能进，**都能卖自己的**；卖价只有系统管理员 · 运营 能改 |
| 玩家仓库 | 直接修改等级 / 金币 / 材料 / 仓库 —— **人在线时当场生效，不用重登**；**「批量发送奖励」**：勾一批人、选物品 / 经验 / 金币、写句留言，发到每个人**游戏里的礼物盒**，玩家自己领（在线的当场提醒）| 系统管理员 · 运营；★ **「修改仓库」只有系统管理员**（运营那颗钮是锁着的）|
| 数据管理 | 定时 / 手动备份，一键回滚；**下载日志**（服务端日志 / 客户端崩溃包打成 zip）| ★ 只有系统管理员 |
| 管理员账号 | 加人、改密码、改权限 | ★ 只有系统管理员 |

> **⚠ 玩家看得到商店价格、合成配方和掉落表。** 这一档就是给玩家当「资料站」用的

七份配置就是 `server/data/` 里的 `items.json` / `shop.json` / `recipe.json` /
`drops.json` / `cards.json` / `rewards.json` / `sell_price.json`，用记事本直接改也行，
一样不用重启。
第一次开服按内置模板生成一份，
...
同一页「保存设置」旁边的**「⇩ 下载日志」**：不用远程登录服务器，在浏览器里就能把日志打成 zip 拿下来。

- **服务端日志 `logs/`**：「下载全量」= 目录第一层的全部文件；「下载最近 12 小时」= 其中最后修改时间在
  12 小时内的那些（正在写的 `server.out` / `online.log` 永远在内）；
- **客户端崩溃包 `logs_client_crash/`**：每份崩溃包一行、可以单独下载，也可以一键下载全部；
- zip 文件名带类别和打包时刻（服务器本地时间）：`logs_server_20260917-213045.zip` /
  `logs_server_12h_…` / `logs_client_crash_…` / `logs_client_crash_<某一份>_…`，
  zip 根上有一份 `MANIFEST.txt` 写着打了哪些文件；
- **边打包边传**，浏览器的下载栏里看得到进度（总大小要传完才知道），服务器上不留副本；
  打包在那条 HTTP 连接自己的线程上做，不影响正在进行的对局。
...
原版客户端把地图、模型、贴图、音效、`.ini` 全锁在加密的 `Pack\*.pkn` 里，无法直接修改。现在这条链已改为：
...
| `server/data/` | 玩家存档 + 七份运营配置 + 发奖记录 + 备份，**都不进 Git** |
...
这几份配置都是热重载的，保存完**下一次读取**就生效，不用重启服务端；
...
**原版就没有这个功能**（那个界面的应答包至今没逆完），所以卖出做在
**GM 管理网页的「装备卖出」页**上 —— 用你自己的游戏账号登
`http://<服务器地址>:27810/admin` 就能卖，卖完游戏里的金币和仓库当场就变。

正在打一局的时候卖不了，会提示「游戏过程中无法卖出」—— 战斗里那份装备加成是
开局时一次性发给各家客户端的，打到一半改它会让每个人算出不一样的伤害。
回大厅或回房间就能卖了。
