...
- 常用命令 例（游戏房间聊天框里输入）：
  ...
  - `/w 2` 可以限制 bot 只能使用 2 号武器。 `/w 0` 表示 bot 可以自由换枪。（通过这个命令可以实现  `2`「手雷大战」 或者 `3`「中门对狙战」的效果。）
  - `/r` 让所有 bot 都进入 **准备** 状态。
  - `/help` 查看命令说明。
