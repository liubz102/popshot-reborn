...
- **GM 管理网页 `/admin`**：管理商店货架 / 合成配方 / 调整掉落材料 / 修改玩家背包
...
| 标签页 | 干什么 | 谁能进 |
|---|---|---|
| 物品库 | 物品的**中文名 / 等级门槛 / 角色限定** —— 这三样只有这一个出处 | 系统管理员 · 运营 |
| 商店货架 | 哪些上架、卖多少钱 | 系统管理员 · 运营 |
| 合成配方 | 产物 / 花费 / 材料（最多 4 种）| 系统管理员 · 运营 |
| 材料掉落 | 哪个模式、哪一关、哪个难度掉什么、掉多少、多大概率 | 系统管理员 · 运营 |
| 玩家资料 | 找人、改等级 / 金币 / 材料 / 背包 —— **人在线时当场生效，不用重登** | ★ 只有系统管理员 |
...
四份配置就是 `server/data/` 里的 `items.json` / `shop.json` / `recipe.json` /
`drops.json`，用记事本直接改也行，一样不用重启。第一次开服按内置模板生成一份，
