...
2. `hook\bshook.dll`（x86）运行时把初始化结果改成成功，按简→繁试：
   - a. 初始化调用点写 `xor eax,eax; nop…`（内存 patch）
   - b. hook `CreateProcessA/W` 吞掉 `GameGuard.des` 的启动
   - c. GameGuard 若被手动映射 → hook 其映射/入口调用
   - d. 短路周期性心跳/检测回调
3. `hook\bsloader.exe`：`CreateProcess(CREATE_SUSPENDED)` → 注入 → `ResumeThread`。
   **patch 时机在 ASProtect 解壳之后**：DllMain 起等待线程，
   轮询目标 RVA 处特征字节出现后再写（解壳前那里是密文）
