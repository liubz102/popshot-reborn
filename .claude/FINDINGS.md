...
## 183. ★★★★★ 双击 A/D 出近身攻击：完整链路 + 「按得越快越发不出」的真凶
...
只放宽 ⑥ 那个窗口常量：`0x515b1d` / `0x515b84` 的 `81 F9 FA 00 00 00`
（`cmp ecx, 250`）→ `81 F9 F4 01 00 00`（`cmp ecx, 500`）。
**攻击频率上限一点没动**（893 ms 锁定和 600 ms 冷却原样保留），
改的只是「你的点按算不算数」：锁一解开客户端会立刻记下当前这一下，
窗口放宽之后你接下来那一下就能配对成功。
